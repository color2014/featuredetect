...
int main(int argc, char** argv)
{
	...
	//打开视频文件
	if (argc == 1)
		if (!(pCapture = cvCaptureFromCAM("person.AVI")))
		{
	...
	while (1)
	{
		...
		//printf("%f", m_FrameNum); //读取视频它一直等于-1.00000
		//逐帧读取视频
		...
		//如果是第一帧，需要申请内存，并初始化
		if (nFrmNum == 1)//设置第一为背景
		{
			pBkImg = cvCreateImage(cvSize(pFrame->width, pFrame->height), IPL_DEPTH_8U, 1);//这里pFrame是从视频获取的第一帧
			...
			cvConvert(pFrImg, pFrameMat);   //#define cvConvert(src, dst )  cvConvertScale((src), (dst), 1, 0 )Parameters:参数	
											//src – Source array   源  源数组
											//	dst – Destination array 目的  目的数组  
											//	scale – Scale factor   比例  比例因数
											//	shift – Value added to the scaled source array elements  转换  资源乘因数加上这个参数
		...
		{
			//printf("%d    %f", nFrmNum, m_FrameNum);
			if ((float)nFrmNum/m_FrameNum==1)//这个判断是最后一帧读取完毕后再循环中再次开启视频，重复播放
			{
			
				pCapture = cvCaptureFromCAM(0); //连续播放视频 2011.06.07
				nFrmNum = 0;
			}
			cvCvtColor(pFrame, pFrImg, CV_BGR2GRAY);
